...
export const AdminOrders = () => {
  ...
  return (
    ...
    <Stack justifyContent={'center'} alignItems={'center'} sx={{pt: { xs: '5rem', md: '4.5rem' }}}>
                ...
                orders.length && orders.map((order,index) => (
                  ...
                  <TableRow key={order._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    ...
                    <TableCell align="right">
                      {
                        order.item?.map((product, pIndex)=>(
                          (()=>{
                            const prod = product?.product || product || {}
                            return (
                              <Stack key={prod._id || pIndex} mt={2} flexDirection={'row'} alignItems={'center'} columnGap={2}>
                                <Avatar src={prod?.thumbnail || ''}></Avatar>
                                <Typography>{prod?.title || prod?.name || 'Item'}</Typography>
                              </Stack>
                            )
                          })()
                        ))
                    ...
                    <TableCell align="right">{formatINR(order.total)}</TableCell>
                    <TableCell align="right">
                      <Stack>
